...
<div class="{{$index==0 ? "tab-pane fade active show":"tab-pane fade hide"}}" id="{{$data->p_seq}}" style="padding-top:15px">
    ...
    @if($data->p_fileoutputcheck==1)
        <div class="row w-100 mx-auto" style="padding-bottom:15px">
            ...
            <div class="form-control form-control-sm col-md-5" readonly>{{'/home/batch/'.$data->p_worklargectg.'/'.$data->p_workmediumctg.'/result/'}}</div><input name="Sc_FileOutputFile" class="form-control form-control-sm col-md-5" type="text"  value="{{explode('.',basename($data->p_file))[0].'_'.$data->sc_seq.'.csv'}}" readonly>
        </div>
         {{-- 개인정보 유무 --}}
