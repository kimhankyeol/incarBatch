...
var FooPicker = (function () {
    ...
    function parse(instance, value) {
      ...
      switch(instance.options.dateFormat) {
        ...
        case 'yyyy-MM-dd':
          dateArray = value.split('-');
          date = new Date(parseInt(parseInt(dateArray[0]), getMonthNumber(dateArray[1]), dateArray[2]));
          return date;
        case 'yyyy.MM.dd':
          dateArray = value.split('.');
          date = new Date(parseInt(dateArray[0]), parseInt(dateArray[1]) - 1, parseInt(dateArray[2]));
        ...
        case 'yyyy/MM/dd':
          dateArray = value.split('/');
          date = new Date(parseInt(dateArray[0]), parseInt(dateArray[1]) - 1, parseInt(dateArray[0]));
          return date;
        case 'yyyy/MM/dd':
          dateArray = value.split('/');
        ...
        case 'yyyy.MM.dd':
          dateArray = value.split('.');
          date = new Date(parseInt(parseInt(dateArray[1]), dateArray[2]), parseInt(dateArray[0]) - 1);
          return date;
        case 'yyyy/MM/dd':
          dateArray = value.split('/');
          date = new Date( parseInt(dateArray[1]), parseInt(dateArray[2]),parseInt(dateArray[0]) - 1);
          return date;
        case 'yyyy-MM-dd':
          dateArray = value.split('-');
          date = new Date(parseInt(dateArray[0]), parseInt(dateArray[1]) - 1, parseInt(dateArray[2]));
      ...
      buildCalendar: function() {
        ...
        for (; day < daysInMonth; day++) {
          if (day < 0) {
            ...
            dayClass = day === (this.today() - 1) ? 'foopicker__day--today' : '';
            template += '<td><div class="foopicker__day ' + dayClass + '" ';
            template += 'data-year="' + this.year() + '" data-month="' + (this.month() + 1);
            template += '"data-day="' + (day + 1) + '" ';
            template += '>' + (day + 1) + '</div></td>';
          }
          columnIndex++;
          if (columnIndex % 7 === 0) {
